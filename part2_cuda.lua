...
trainData = {
  data = torch.CudaTensor(6750, 1, 128, 128),
  labels = torch.CudaTensor(6750, 2, 128, 128),
...
testData = {
  data = torch.CudaTensor(75, 1, 128, 128),
  labels = torch.CudaTensor(75, 2, 128, 128),
...
batchSize = 5
-- push model and criterion onto GPU
criterion:cuda()
model:cuda()
...
final_result = torch.Tensor(5,3,128,128)
for i = 1,5 do
  final_result[i]:copy(image.lab2rgb(output_lab[i]))
end
image.display(final_result)
image.display(train_images[{{1,5},{},{},{}}])
